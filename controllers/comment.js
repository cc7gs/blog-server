 ...
 * @param {*} opts 
 */
export const putComment = async (ctx, comment) => {
  // 获取ip地址以及物理地理地址
  const ip = (
  ...
  let permalink = 'https://ccwgs.top/about'
  let article = await Article.findById({ _id: comment.post_id })
  article.meta.comments += 1
  permalink = `https://ccwgs.top/article/${article._id}`
  // 发布评论
 ...
 * @param {修改参数} opt 
 */
export const editeComment = async (_id, opt) => {
 ...
 * @param {修改参数} opt 
 */
export const likeComment = async (_id) => {
 ...
 * @param {文章id} _id 
 */
export const getComment = async (opts = {}) => {
